...
export class SwapHelper {
  ...
  public async loadReserves(path: string[]): Promise<Array<IReserve>> {
    ...
    for (let i = 1; i < path.length; i++) {
      const currLp = PancakePair__factory.connect(
        await this.factory.getPair(path[i - 1], path[i]),
        this.signer
      );
  ...
  public async computeLpHealth(
    liquidity: BigNumber,
    tokenA: string,
    tokenB: string
  ): Promise<BigNumber> {
    const [tokenAamount, tokenBamount, rA, rB] = await this.computeUnderlaying(
      liquidity,
      tokenA,
      tokenB
    );
    return tokenAamount.add(
      this.getMktSell(tokenBamount, rB.sub(tokenBamount), rA.sub(tokenAamount))
    );
  }
  ...
  public async computeRemoveLiquidiy(
    ...
    const currLp = PancakePair__factory.connect(
      await this.factory.getPair(tokenA, tokenB),
      this.signer
    );
    const reserveIdx = this.reserves.findIndex((r) => r.lp === currLp.address);
    if (reserveIdx === -1) {
    ...
    this.reserves[reserveIdx].r0 = this.reserves[reserveIdx].r0.sub(
      !isReversed ? amountA : amountB
    );
    this.reserves[reserveIdx].r1 = this.reserves[reserveIdx].r1.sub(
      !isReversed ? amountB : amountA
    );
  ...
  public async computeAddLiquidity(
    ...
    this.reserves[reserveIdx].r0 = this.reserves[reserveIdx].r0.add(
      !isReversed ? amountA : amountB
    );
    this.reserves[reserveIdx].r1 = this.reserves[reserveIdx].r1.add(
      !isReversed ? amountB : amountA
    );
  ...
  public async computeReinvestLp(
    ...
    [
      reinvestFees: BigNumber,
      reinvestLp: BigNumber,
      debrisBtoken: BigNumber,
      debrisFtoken: BigNumber
    ]
  > {
    ...
    const reinvestAmts = await this.computeSwapExactTokensForTokens(
      reinvestLeft,
      reinvestPath,
      true
    );
    return [
      reinvestFees,
      ...(await this.computeOneSidedOptimalLp(
        reinvestAmts[reinvestAmts.length - 1].add(debrisBtoken),
        path
      )),
    ];
  }
  ...
  public async computeTwoSidesOptimalLp(
    ...
    const [swapAmt, isReversed] = await this.computeTwoSidesOptimalSwap(
      btokenAmount,
      ftokenAmount,
      path
    );
