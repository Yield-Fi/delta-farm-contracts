...
contract PancakeRouterV2 is IPancakeRouter02 {
  ...
  function _addLiquidity(
    ...
    (uint256 reserveA, uint256 reserveB) = PancakeLibraryV2.getReserves(
      factory,
      tokenA,
      tokenB
    );
    if (reserveA == 0 && reserveB == 0) {
      ...
      uint256 amountBOptimal = PancakeLibraryV2.quote(
        amountADesired,
        reserveA,
        reserveB
      );
      if (amountBOptimal <= amountBDesired) {
        ...
        uint256 amountAOptimal = PancakeLibraryV2.quote(
          amountBDesired,
          reserveB,
          reserveA
        );
    ...
    )
  {
    ...
    if (msg.value > amountETH)
      SafeToken.safeTransferETH(msg.sender, msg.value - amountETH);
  }
  ...
  function _swap(
    ...
    for (uint256 i; i < path.length - 1; i++) {
      ...
      address to = i < path.length - 2
        ? PancakeLibraryV2.pairFor(factory, output, path[i + 2])
        : _to;
    ...
    returns (uint256[] memory amounts)
  {
    ...
    if (msg.value > amounts[0])
      SafeToken.safeTransferETH(msg.sender, msg.value - amounts[0]);
  }
  ...
  function _swapSupportingFeeOnTransferTokens(
    ...
    for (uint256 i; i < path.length - 1; i++) {
      ...
      IPancakePair pair = IPancakePair(
        PancakeLibraryV2.pairFor(factory, input, output)
      );
      uint256 amountInput;
      uint256 amountOutput;
      {
        ...
        amountOutput = PancakeLibraryV2.getAmountOut(
          amountInput,
          reserveInput,
          reserveOutput
        );
  ...
  function swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    uint256 balanceBefore = IERC20Upgradeable(path[path.length - 1]).balanceOf(
      to
    );
    _swapSupportingFeeOnTransferTokens(path, to);
    require(
      IERC20Upgradeable(path[path.length - 1]).balanceOf(to).sub(
        balanceBefore
      ) >= amountOutMin,
      "PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT"
    );
